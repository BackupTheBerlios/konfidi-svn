...
Konfidi refers to the trust network design, the ontology used to encode it, and the software to make it usable.  Figure~\ref{fig:Arch} shows the components of the Konfidi architecture and how they relate to external components and one another.  The numbered paths indicate the steps in the process:  first, a client makes a request using the Konfidi URL query language, indicating the source, usually the person making the inference, and the sink, the person that the path leads to, the trusted party.
