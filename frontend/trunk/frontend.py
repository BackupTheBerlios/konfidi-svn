	...
	def command(self, req):
		if (req.method == "POST" or req.method == "GET"):
			...
			if (cmd == "start"):
				...
				(stdin, stdout, stderr) = os.popen3("cd /home/ams5/public_html/tests/prototype && ../../clients/simple/load_rdf.py | python")
				req.write("std out: %s\n" % stdout.read())
				req.write("std err: %s\n" % stderr.read())
			elif (cmd == "load2"):
				(stdin, stdout, stderr) = os.popen3("cd /home/ams5/public_html/tests/small/ && ../../clients/simple/load_rdf.py |  python")
				req.write("std out: %s\n" % stdout.read())
				req.write("std err: %s\n" % stderr.read())
			elif (cmd == "load3"):
				(stdin, stdout, stderr) = os.popen3("cd /home/ams5/public_html/tests/edgecases/ && ../../clients/simple/load_rdf.py | python")
				req.write("stdout: %s\n" % stdout.read())
				req.write("stderr: %s\n" % stderr.read())
			else:
				pass
